function output = prt_machine_bayesMKL_reg(d,def,args,args_optional)
...
res = bemkl_supervised_regression_variational_test(ktrain,state);
func_val_train = res.y.mu + m;
%fprintf('----- bayesMKL: %i kernels, train correlation %f ------\n',size(ktrain,3),corr2(func_val_train,tr_targets));
