function output = prt_machine_sMKL_reg(d,def,args,args_optional)
...
if nargin==4 && ~isempty(args_optional)
    opt_fields = fields(args_optional);
    for k=1:length(opt_fields)
        options.(opt_fields{1}) = args_optional.(opt_fields{1});
    end    
end
...
%--------------------------------------------------------------------------
C_opt = args;
options.sigmainit = 1/size(d.train,3)*ones(1,size(d.train,3)); %initialize kernel weights
...
ktrain_optimal = zeros(size(ktrain,1));
for i = 1:size(d.train,3)
    ktest_final = ktest_final + beta(i)*ktest(:,:,i);
    ktrain_optimal = ktrain_optimal + beta(i)*ktrain(:,:,i);
end

func_val = ((ktest_final*alpha)+b)+m; % add mean from the training set

%func_val_train = ((ktrain_optimal*alpha)+b); % add mean from the training set
%fprintf('----- simpleMKL: %i kernels, C %f, train correlation %f ------\n',size(ktrain,3),C_opt,corr2(func_val_train,tr_targets));
